...
__all__ = ['AdaptiveConcatPool3d', 'get_hps', 'inflate', 'create3Dhead', 'create3Dcnn']
...
def create3Dhead(n_in, n_mid, n_out):
    return nn.Sequential(AdaptiveConcatPool3d(),
                         Flatten(),
                         nn.BatchNorm1d(n_in),
                         nn.Dropout(p=.25),
...
def create3Dcnn(model=resnet18, cut=-2, n_in=512, n_mid=50, n_out=10):
    encoder, head = inflate(create_body(model, cut=cut)).eval(), create3Dhead(n_in*2, n_mid, n_out).eval()
    return TLModel(encoder, head)