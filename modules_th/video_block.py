...
__all__ = ['show_frames', 'DF2Paths', 'Video', 'snippets_from_video', 'stretch', 'ResizeTime', 'encodes', 'TensorVideo',
           'encodes', 'encodes', 'UniformizedDataLoader', 'uniformize_dataset', 'UniformizedShuffle', 'DF2Paths',
           'VidToTensor', 'create_video', 'createVideoForm', 'RandomCrop', 'RandomHFlip', 'RandomColorJitter']
...
class VidToTensor(Transform):
    def encodes(self, vid):
        vid = L(map(ToTensor(),vid)).stack().permute(3,0,1,2)
        return TensorVideo(vid)

# Cell

def create_video(vid_path, start=None, l=50 , skip=3, form='tens'):
    assert form in ['tens', 'img'], "form should be either 'tens' or 'img'"
    assert  os.path.exists(vid_path), f'The video path does not exist {vid_path} '
    vidcap = cv2.VideoCapture(str(vid_path))
    duration = vidcap.get(cv2.CAP_PROP_FRAME_COUNT)
    vid, block = L(), l*skip

    if start is None:  start = random.randint(0, max(0,duration - block))
    i = 0
    while len(vid) < l:
        check = start + i*skip
    ...
    else: return Video(vid.map(PILImage.create))
